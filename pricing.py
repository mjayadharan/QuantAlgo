...
class PricingEnvironment:
    ...
    def load_data_from_excel(self, file_names):
        for file_name in file_names:
            try:
                ...
                print(f"Loaded data for asset '{asset_name}' from {file_name}.xlsx with {len(data)} rows") 
        ...
        for file_name in file_names:
            try:
                ...
                if isinstance(data['Timestamp'].iloc[0], str):
                    data['Timestamp'] = pd.to_datetime(data['Timestamp'])
                self.assets[asset_name] = data.set_index('Timestamp')['Open'].to_dict()
                print(f"Loaded data for asset '{asset_name}' from {file_name}.csv with {len(data)} rows") 
            except Exception as e:
    ...
    def get_open_price_for_index(self, index):
        open_prices = {}
        for asset, prices in self.assets.items():
            try:
                # Convert index to integer
                index = int(index)
                # Check if index exists in prices dictionary
                if index < len(prices):
                    open_prices[asset] = list(prices.values())[index]
                else:
                    print(f"Index '{index}' out of range for asset '{asset}'")
            except ValueError:
